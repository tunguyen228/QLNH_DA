...
const GiaoDienThanhToanContent = () => {
    ...
    // Lấy danh sách bàn đang có phiếu gọi chưa thanh toán
    const fetchTables = async () => {
        ...
        try {
            ...
            addToast("Không thể tải danh sách bàn", "danger");
        } finally {
            setLoadingTables(false);
        }
    };

    // Tải danh sách bàn lần đầu khi vào trang
    useEffect(() => {
        fetchTables();
    }, []);

    // Dùng chung connection SignalR từ NotificationProvider (layout) thay vì tự mở connection riêng
    useEffect(() => {
    ...
    // Lấy danh sách món trong phiếu gọi khi chọn bàn
    useEffect(() => {
        if (!selectedTable) {
            setOrderItems([]);
            return;
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            const payload = {
                MaBan: parseInt(selectedTable, 10),
                SoDienThoai: "",
            ...
            if (result.success || result.Success) {
                ...
                // Cập nhật lại danh sách bàn (bàn vừa thanh toán sẽ biến mất)
                await fetchTables();
            } else {
                addToast(`Lỗi: ${result.message || result.Message}`, "danger");
            }
        } catch (error) {
            addToast("Lỗi kết nối đến máy chủ khi thanh toán.", "danger");
        } finally {
                            ...
                            <Col md={4} className="bg-white d-flex flex-column h-100 shadow-sm p-4">
                                ...
                                <div className="mt-auto pt-3 border-top">
                                    ...
                                    <Button
                                        ...
                                        disabled={!selectedTable || orderItems.length === 0 || isGeneratingInvoice}
                                    >
                                ...
                                {paymentMethod === 'qr' && (
                                    <Card className="border-0 shadow-sm text-center p-3 flex-grow-1 d-flex flex-column justify-content-center align-items-center bg-light rounded-4">
                                        <div className="bg-white p-2 rounded-4 mb-3 shadow-sm">
                                            <img src={vietQrUrl} alt="VietQR" style={{ width: '250px', height: 'auto', objectFit: 'contain' }} />
                                        </div>
                            ...
                            <Col md={5} className="bg-light border-start p-4 h-100 overflow-auto">
                                ...
                                <Card className="border-0 shadow-sm p-4" style={{ fontFamily: 'monospace', fontSize: '0.9rem' }}>
                                    <div className="text-center mb-4">
                                        ...
                                        <small className="d-block">Thu ngân: {tenThuNgan}</small>
                                    </div>
                                    <hr style={{ borderStyle: 'dashed' }} />
                                    <div className="d-flex justify-content-between fw-bold mb-2">
                                        <span>{tables.find(t => t.id === parseInt(selectedTable, 10))?.name || `Bàn ${selectedTable}`}</span>
                                    ...
                                    <div className="text-center mt-4 text-muted small fst-italic">"Cám ơn Quý khách đã ủng hộ nhà hàng.<br/>Hẹn gặp lại quý khách lần sau!"</div>
                                </Card>
                            </Col>
                        </Row>
                    )}
